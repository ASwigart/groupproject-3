...
def arrivals():
    conn = connect_to_db1()
    posts = conn.execute('SELECT * FROM arrivals'). fetchall()
    conn.close()
    return render_template('arrivals.html', year='year', number='number') 
...
# def index():
#     conn = connect_to_db1()
#     arrive = conn.execute('SELECT * FROM arrivals'). fetchall()
#     conn.close()
#     return render_template('index.html', arrive = arrive) 
# def lite_get_db1():    
#         arrivals = {
#             "year": "year",
#             "number": "number"
